...
#define COOLINGRainbow  70 // 100 // 55 // 86
    ...
    for(uint8_t x = 0; x < MATRIX_WIDTH; x++) {
      ...
      for (int j = 0; j < MATRIX_HEIGHT; j++) {
          ...
          byte colorindex = scale8(heat[x][j], 160);
           leds[XY(x, j)] = ColorFromPalette(RainbowColors_p, colorindex);
          // leds[XY(x, (MATRIX_HEIGHT -1) - j)] = ColorFromPalette(RainbowColors_p, colorindex); //REVERSE DIRECTION
      }
    }
